...
public class AcademicSemesterRepository : IAcademicSemesterRepository
{
    ...
    public AcademicSemesterRepository(DbContextOptions<ApplicationDbContext> options, IMapper mapper)
    {
        _mapper = mapper;
        dbOptions = options;
    }
    ...
    public  async Task<IEnumerable<SemesterDto>> GetActiveSemester()
    {
        try
        {
            using (var db = new ApplicationDbContext(dbOptions))
            {
                var result = db.Semester.Where(x => x.isActive);
                IEnumerable<SemesterDto> SemesterDto = _mapper.Map<IEnumerable<Semester>, IEnumerable<SemesterDto>>(result);
                return SemesterDto;
            }
        }
        catch (Exception ex)
    ...
    public async Task<IEnumerable<SemesterDto>> GetActiveApplicantSemester()
    {
        try
        {
            using (var db = new ApplicationDbContext(dbOptions))
