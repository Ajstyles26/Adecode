...
{
    public class ApplicantPayScheduleService : IApplicantPayScheduleService
    {
        IApplicantPayUploadCategoryBatchRepository _PUCBRepository;
        IApplicantPayUploadCategoryRepository _PUCRepository;
        public ApplicantPayScheduleService(IApplicantPayUploadCategoryBatchRepository PUCBRepository, IApplicantPayUploadCategoryRepository pUCRepository)
        {
            _PUCBRepository = PUCBRepository;
            _PUCRepository = pUCRepository;

        }
        public async Task<IEnumerable<ApplicantPayUploadCategoryDto>> ProcessCSV(byte[] fileBytes)
        {
            var paymentSchedules = new List<ApplicantPayUploadCategoryDto>();
            ...
            using (var package = new ExcelPackage(stream))
            {
                ...
                //create your batch 
                ApplicantPayUploadCategoryBatchDto currentPUCBDto = new ApplicantPayUploadCategoryBatchDto();
                currentPUCBDto.Name = $"Applicant Schedule Batch {DateTime.UtcNow}";
                ...
                {
                    var paymentSchedule = new ApplicantPayUploadCategoryDto
                    {
                        ...
                        IsGlobal = bool.Parse(worksheet.Cells[row, 4].Value.ToString().ToLower()),
                        ...
                        ApplicantPayUploadCategoryBatchId = svdcurrentPUCBDto.Id
                    };
        ...
        public async Task<IEnumerable<ApplicantPayUploadCategoryDto>> ProcessCSVWithTimeout(byte[] fileBytes, TimeSpan timeout)
        {
